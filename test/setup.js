...
module.exports = async (FileAdapter, fileName) => {
  await rimrafAsync(path.resolve(__dirname, '*.json'))

  const dataFile = path.resolve(__dirname, fileName)
  ...
  describe('initialize', () => {
    it('load', async () => {
      const newFile = path.resolve(__dirname, 'new-' + fileName)
      const tempStore = new Persiston(new FileAdapter(newFile))
  
      await tempStore.load()
  
      const content = await readFileAsync(newFile, 'utf8')
